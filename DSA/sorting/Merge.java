...
public class Merge {
    public void merge(int[] arr, int low, int mid, int high) {
        // OPTIMIZATION: Using a primitive int[] array instead of ArrayList<Integer>
        // to avoid Object overhead, GC pressure, and Cache Misses.
        ...
        while (left <= mid && right <= high) {   //compares both sorted array
            if (arr[left] < arr[right]) {      //for smaller element and appends
                temp[tempIdx++] = arr[left++]; // it to temp, if [left] is bigger then
            } else {
                temp[tempIdx++] = arr[right++];// appends element at [right] index
            }
        }
        ...
        for (int i = low; i <= high; i++) {
            arr[i] = temp[i - low];          //like array with 0 to 14 indexes
    ...
    void ms(int[] arr, int low, int high) {
        ...
        //find mid of array
        // FIX: Replaced (low + high) / 2 to prevent integer overflow on massive arrays!
